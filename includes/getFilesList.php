...
if(isset($_GET['ID_Kurs']))
{
    ...
    if(!empty($result)) {
        foreach ($result as $row) {
            echo "<li><a href=\"file-download.php?file_id={$row->ID_Plik}&file_name={$row->nazwa}&url={$row->sciezka}&kurs_id={$getID}&user_id={$_SESSION['ID_USER']}\" class=\"items\">".$row->nazwa."</a></li>";
        }
    }
    else
    {
